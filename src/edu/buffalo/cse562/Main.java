...
public class Main {
   ...
   * @throws FileNotFoundException when a given file is invalid
   */
  private static LinkedHashSet<File> cli(String[] args) throws FileNotFoundException {
    ...
    for (int i = 0; i < args.length; i++) {
      if (args[i].equals("--data")) {
        Database.setDataDirectory(args[i + 1]);
        i++;
      } else {
        File file = new File(args[i]);
        if (file.exists()) files.add(file);
        else throw new FileNotFoundException(args[i] + " is not a valid file!");
      }
    }
